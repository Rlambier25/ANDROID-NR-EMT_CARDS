...
import eu.davidea.flipview.SVGPictureDrawable;
...
public class FlipView extends ViewFlipper implements SVGPictureDrawable, View.OnClickListener {
    ...
    public void addRearLayout(@NonNull View view) {
        ...
        } else if (viewGroup.getChildAt(whichChild) instanceof TextView) {
            Log.d(TAG, "Found TextView in RearLayout");
            rearText = (TextView) viewGroup.getChildAt(whichChild);
        }
    ...
    public void setRearText(CharSequence text) {
        if (this.rearText == null) {
            Log.w(TAG, "TextView not found in the first child of the FrontLayout. Text cannot be set!");
            return;
        }
        rearText.setText(text);
    }
